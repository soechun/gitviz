...
var svg = d3.select("#svg"),
    margin = {top: 20, right: 80, bottom: 1000, left: 50},
...
function drawParticipationGraph(data) {
  var keys = data.columns.slice(1, 3);
    console.log(data.map(function(d) { return d.week; }));
    var userCategories = data.columns.slice(1).map(function(id) {
      return {
        id: id,
    ...
    y.domain([
      d3.min(userCategories, function(c) { return d3.min(c.values, function(d) { return d.commitCount; }); }),
      d3.max(userCategories, function(c) { return d3.max(c.values, function(d) { return d.commitCount; }); })
    ]);

    z.domain(userCategories.map(function(c) { return c.id; }));
